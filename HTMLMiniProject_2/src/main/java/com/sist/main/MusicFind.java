...
public class MusicFind extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 1. 브라우저로 전송 HTML 을 시작한다
				...
				if (startPage > 1) {
					out.println("<li><a href=\"MainServlet?mode=4&type=" + column + "&fd=" + fd + "&page=" + (startPage - 1) + "\">&lt;</a></li>");
				}
				/*
				...
				for (int i = startPage; i <= endPage; i++) {
					if (i == curPage) {
						out.println("<li class=active><a href=\"MainServlet?mode=4&type=" + column + "&fd=" + fd + "&page=" + i + "\">" + i + "</a></li>");
					} else {
						out.println("<li><a href=\"MainServlet?mode=4&type=" + column + "&fd=" + fd + "&page=" + i + "\">" + i + "</a></li>");
					}
				}
				if (endPage < totalPage) {
					out.println("<li><a href=\"MainServlet?mode=4&type=" + column + "&fd=" + fd + "&page=" + (endPage + 1) + "\">&gt;</a></li>");
				}
